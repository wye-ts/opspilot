...
| Data source | Frontend-known request-lifecycle stages (`availability`\* /`job`/`run`/`approval`), derived in `investigation-progress/investigation-progress-stages.ts` | Dual-sourced as of Issue #56 — see §6.2 |
| Available | **Immediately** on submit, before any request has resolved | Only once `run !== null` — i.e. after the run response arrives |
| Status vocabulary | `Pending` / `Active` / `Completed` / `Failed` — never a percentage, never advanced by a timer | `completed` / `running` / `failed` / `neutral` glyphs — no live/pending concept for the array as a whole (the whole array arrives at once) |
...
**Dual-sourced as of Issue #56.** `TraceTimeline` accepts `trace` (the legacy `AgentTraceEvent[]`, unchanged shape and meaning) plus two new optional props: `events` (the canonical `InvestigationEventRecord[]`, the same `events` field `App.tsx` already holds for the Progress Timeline) and `executionStageDerivation` (the SAME reducer-validated `ExecutionStageDerivation` value `App.tsx` already computes for the Progress Timeline — never re-derived a second time here). The component branches on `executionStageDerivation.kind`, never on the weaker `hasCanonicalInvestigationLifecycleMarker` origin-only check alone:

- **`"legacy"`** (the default when the prop is omitted, matching `App.tsx`'s own default for an unwired call site) — renders `trace` exactly as this component always has. Every pre-#56 caller, including every pre-#56 test, is unaffected.
- **`"canonical"`** — renders `events` through the richer 13-type canonical vocabulary (§ below), reducer-confirmed valid.
- **`"canonical-invalid"`** — renders a fixed "Agent activity detail isn't available for this run right now." message and nothing else: never a fallback to `trace` (which would silently fabricate a plausible-looking legacy timeline for data the reducer has already rejected — the same anti-pattern the Progress Timeline's own `canonical-invalid` handling already avoids) and never a crash. This message is deliberately its OWN wording, not the Progress Timeline's identical-condition note — the two timelines are documented as independent surfaces with their own vocabularies (§6.1), and Agent Activity has no "frozen last-good events" concept the way the Progress Timeline's stage rows do.

**Ordering source:** array order, exactly as the API returns it (`sequence_number ASC` for canonical `events`; unchanged for legacy `trace`), never re-sorted by the UI.

**No per-event timestamps.** Neither `AgentTraceEventSchema` nor `InvestigationEventRecordSchema`'s canonical facts carry a rendered time field beyond `recordedAt` (unused here). The timeline uses array-order ordinals only. Real timestamps (`startedAt`/`finishedAt`/duration) appear only in the Investigation summary panel, where they are genuine.

**Vocabulary — legacy path (`trace`), 4 types, unchanged:** `TOOL_REQUESTED`, `TOOL_COMPLETED`, `REPORT_GENERATED`, `RETRIEVAL_COMPLETED` (the last is currently unreachable through `apps/api`, since no retriever is wired into its tool registry — it is implemented because an exhaustive switch over the contract union requires it).

**Vocabulary — canonical path (`events`), 13 types.** `presentInvestigationActivityLabel` (`trace/trace-product-labels.ts`) is the canonical counterpart to the legacy path's `presentTraceProductLabel`, and reuses byte-identical wording for every type both paths share (no tone drift): `TOOL_REQUESTED`/`TOOL_COMPLETED` for the grounded `get_service_status` tool render **"Checking service status"**/**"Checked service status"**, `RETRIEVAL_COMPLETED` renders **"Runbook retrieval completed"** with a grounded chunk-count detail, and `REPORT_VALIDATED` renders **"Resolution report generated"** — the exact same copy as legacy `REPORT_GENERATED`, since it is documented to mean exactly what that legacy type has always meant. 7 types are new, previously invisible to Agent Activity entirely: `RUN_CREATED` ("Investigation created"), `AGENT_STARTED` ("Agent started analyzing the ticket"), `TOOL_FAILED` ("Checking service status — failed" for the grounded tool, generic "Diagnostic tool failed" otherwise — the first time a tool failure has ever been visible on this surface), `REPORT_GENERATION_STARTED` ("Preparing the final report"), `REPORT_SUBMITTED` ("Report submitted for validation"), `REPORT_VALIDATION_FAILED` ("Report failed validation"), `RUN_COMPLETED`/`RUN_FAILED` ("Investigation completed"/"Investigation failed", closing narrative lines — not redundant with the Progress Timeline's own terminal state, because Agent Activity is a chronological log, not a status widget).

**Unknown-event fallback (both paths):** an event type outside the current union renders the generic product phrase "Agent activity recorded" with the raw `type` string as supporting detail only, never a raw payload dump. `presentInvestigationActivityLabel` is an exhaustive switch with an `assertNever`-style default, matching `packages/contracts/src/investigation-lifecycle-compatibility.ts`'s `projectToLegacyAgentTraceEvent` pattern — a future 14th canonical type fails to compile here until it is given an explicit choice.

**Status glyphs (both paths).** `completed` (✓), `running` (●, a tool request whose matching completion has not arrived yet), `failed` (✕, Issue #56 — a tool request whose matching `TOOL_FAILED` HAS arrived, or a canonical event that is itself a terminal/validation failure fact), `neutral` (•, everything else). The canonical path gives every one of its 7 new types (plus the reused 6) an intentional status — `REPORT_VALIDATED`/`RUN_COMPLETED` resolve `completed` (matching legacy `REPORT_GENERATED`'s existing glyph exactly — a real bug caught during implementation would otherwise have regressed this one row from ✓ to a neutral dot), `TOOL_FAILED`/`REPORT_VALIDATION_FAILED`/`RUN_FAILED` resolve `failed`, and the 4 remaining lifecycle-only markers (`RUN_CREATED`/`AGENT_STARTED`/`REPORT_GENERATION_STARTED`/`REPORT_SUBMITTED`) resolve `neutral`. A `TOOL_REQUESTED` whose matching outcome is a later `TOOL_FAILED` (not a `TOOL_COMPLETED`) now correctly resolves `failed` rather than staying `running` forever — a pre-existing bug in the legacy-path status function, invisible before this issue because `TOOL_FAILED` was never rendered by Agent Activity at all.

**Raw identifiers live behind Technical details.** `traceTechnicalEntries` collects one sanitized row per `TOOL_REQUESTED` (tool name + call id) from whichever stream is active, rendered in a `TechnicalDetails` disclosure under the timeline — never secrets, tokens, prompts, payloads, or stack traces. The primary copy stays product language.
