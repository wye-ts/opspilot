...
API from a clean, cookie-free session. Desktop layout (~1440px) was verified live; the
mobile/narrow-viewport screenshot could not be captured in this session due to a
browser-automation tooling limitation — see §9.
...
| Decision persists after refresh | See §7 — the honest result is API-verified persistence, not a browser-restorable run | Confirmed via API, not via browser (browser has no router/client storage) | §7 |
| Desktop layout usable | Two-column layout, report dominant | Confirmed at ~1418–1440px effective width | screenshots 1–4 |
| Mobile layout usable | Single-column layout below the `64rem`/1024px breakpoint | **Not captured live** — browser-automation window resize was unreliable in this session (see §9); responsive breakpoint confirmed only via source inspection of `apps/web/src/styles.css` | §9 |
...
## 8. Cold-start and warm observations
...
- The deployed browser workflow uses the deterministic `FAKE` provider only. A real-Claude
  provider spike and controlled LIVE validation exist and are documented separately — see §12.
...
- No historical-run browser: a completed run is not reachable through the UI after a
  refresh; it is reachable only through the public, unauthenticated API (§7).
- Free-tier cold starts apply; see §8.
- All data in the public demo is synthetic demo data with no privacy expectation.
- **Mobile-viewport screenshot not captured.** The browser-automation environment used for
  this evidence pass could not reliably resize the browser window to a narrow viewport
  (repeated resize requests to 390×844 and other sizes were not honored — the reported
  `window.innerWidth` drifted unpredictably between ~800px and ~1600px across attempts,
  independent of the requested size). Rather than fabricate a mobile screenshot or crop a
  desktop one, this is left as an open item. The single-column responsive behavior below the
  `64rem` (1024px) breakpoint is confirmed only via source inspection of
  `apps/web/src/styles.css`, not a live observation. A follow-up pass with a working device
  emulator (e.g. Chrome DevTools responsive mode used directly, rather than through this
  automation path) should capture `pending-approval-mobile.png` and confirm R1–R8-style
  responsive/accessibility checks (breakpoint switch point, no horizontal overflow, keyboard
  activation of the approval banner, reduced-motion behavior) that this pass did not attempt.
- Case-insensitive `/V1/**` routing boundary checks and reduced-motion/keyboard-focus checks
  from the original PR 5D plan (`docs/reviews/17-live-demo-evidence-plan.md`) were not
  re-run in this pass — this evidence pass followed a narrower, explicitly scoped checklist
  (root page, health endpoints, provider default, one normal run, one approval-demo run,
  persistence, desktop layout). They remain open verification items if a future pass wants
  full parity with the original plan's matrix.
...
refresh in the browser" (it does not — §7); any phrasing implying a live check identified
the database vendor (that is configuration-derived, not observed).
...
reproduced with any redaction beyond what's already shown above. Screenshots were captured via
browser automation (Claude in Chrome) at native screenshot resolution and converted to PNG;
each was manually inspected before being committed (see §9 for the one check — mobile
viewport — this pass could not complete).
