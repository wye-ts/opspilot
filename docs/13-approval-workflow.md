...
- **Net effect:** this feature's entire footprint outside `apps/api` is `packages/contracts` (2 schemas) and `packages/database` (schema/migration/errors/types/mappers/repository) — `packages/agent-runtime` remains unchanged. **Corrected in Revision 6:** an earlier draft of this paragraph additionally claimed nothing under `apps/api/src/execution/` would change either — that became stale the moment §14's opt-in demo fix was written into this same document, and PR 3B (§17) confirmed it as implemented. The precise claim is narrower: execution DI/runtime composition remains unchanged — `execution.tokens.ts` gains no new entry, and `AgentRuntimeModule`/`DeterministicExecutionModule` gain no new provider or wiring change, so the orchestrator has no approval dependency of any kind. Only `deterministic-provider-factory.ts` and its test gain the opt-in `TICKET-APPROVAL-DEMO` scenario described in §14 — an unrelated, narrowly-scoped change to that file's existing responsibility (constructing the fake-provider scenario), not an expansion of this feature's DI footprint.
