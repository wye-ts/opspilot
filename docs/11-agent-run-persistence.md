...
```
`retryFinalization({ runId, agentResult, usageSummary })` is the explicit retry path for the `finalization` stage — see §10 for its exact durability limit. `usageSummary` is **required**, not optional: the first attempt already measured the run's tokens and cost and reconciled the day's budget from those figures, so a retry that could omit them would finalize the run with `NULL` usage columns while the budget row said it cost money. It is never re-snapshotted (the collector belongs to an execution that has ended) and never re-reconciled (`reservation` stays `null`, which is what tells the caller's cleanup block there is nothing left to do). A FAKE retry passes `usageSummary: null`, which is the honest value for a run that made no provider call.
