...
| Max output tokens/attempt | **Hard, report-safe** | Per **provider attempt**, the report-safe `finalizationMaxOutputTokens` on EVERY provider turn (`MAX_PROVIDER_TURNS = 5`) — issue #61 Codex MAJOR 1, because `submit_resolution_report` is available on investigation turns too. Daily envelope = `finalizationMaxOutputTokens × 5 × (maxRetries + 1) × dailyRunLimit` = `3072 × 5 × 1 × 10` = **153,600** output tokens/day at shipped defaults, where `(maxRetries + 1) = 1` only because the protected path requires `ANTHROPIC_MAX_RETRIES=0`. |
