...
**These are templates, not current claims, and most are not earned as written.** This section was written before implementation began; the project is now implemented through Milestone 13, so each bullet below must be checked against what actually shipped before it is used. Four of the five are annotated inline — two describe functionality this repository does not have, one is accurate only because its verb is "Designed," and one is partly earned. Do not publish any bullet before the underlying work, tests, and measurements actually exist (see `docs/03-technical-design.md §31 Portfolio and Resume Deliverables`, which requires resume claims to use measured results, never invented numbers).

Planned resume description:

**OpsPilot — AI Support & Incident Resolution Agent**

Built a production-style AI agent using Claude, NestJS, React, and PostgreSQL, with tool-calling workflows to triage support tickets, retrieve operational runbooks, run bounded read-only diagnostics, generate resolution plans, and persist one auditable human APPROVE/REJECT decision against a completed run's proposed-action set.

> **Changed from the original draft, which is not accurate for the shipped system.** It named `pgvector` and "inspect simulated logs." Neither shipped: the deployed retriever is in-process keyword/token-overlap scoring over an in-memory corpus (`InMemoryKeywordRunbookRetriever`, the only retriever `apps/api` constructs; BM25 and frozen-embedding retrievers exist as evaluated offline candidates and are not wired into any deployed path — `docs/reviews/31-issue-76-comparison-decision.md`), pgvector appears nowhere outside design documents, and the shipped diagnostic tool catalog contains two tools, `get_service_status` and `get_recent_deployments` (issue #93) — there is no log-search tool. It also said "require human approval **before state-changing actions**," which claims an enforced pre-action gate. Nothing is gated: no action-execution path exists (`README.md` capability matrix: *Action execution — Not implemented*), and the workflow "**records a decision only** — it never executes, simulates executing, or schedules execution" (`docs/13-approval-workflow.md` §1). What shipped is a single decision row per run — *at most one row* is database-enforced (`UNIQUE(run_id)`), while the decision's immutability is an **application-layer** guarantee (no UPDATE/DELETE code path exists in the repository or API; nothing in the schema prevents direct SQL — `docs/13-approval-workflow.md` §5's Revision-3 scope note). It is an audit record, not a control.
...
* **Not earned as written.** "Implemented runbook retrieval with PostgreSQL, pgvector, document chunking, embeddings, and citation-grounded responses." Citation-grounded retrieval shipped and is measured (`recall@k`, MRR, false-positive rate — Milestone 13). `pgvector` and deterministic document chunking did not; precomputed embeddings exist only as a committed **offline comparison fixture** (`runbooks-eval/embedding-fixture.json` + `FixtureBackedRunbookRetriever`), never as a deployed or database-backed embedding store (`docs/05-rag-design.md` "Deferred"). An accurate version claims measured in-process keyword retrieval with a labeled 40-query evaluation set, plus a recorded three-way comparison against BM25 and frozen-embedding candidates that kept the incumbent — which is stronger evidence than the unearned version, not weaker. Do not describe the deployed retriever as BM25 or embedding-based: both were comparison candidates and lost.
* **"Designed" only — accurate as written, and must stay that way.** "Designed modular read-only diagnostic tools for log search, service health checks, account lookup, runbook retrieval, and similar-incident search, plus typed human-approved action proposals for escalations, ticket updates, and customer-response drafts." The five-tool set is designed in `docs/03-technical-design.md §14.3`; exactly one of those five (`check_service_status`, shipped as `get_service_status`) is implemented. A second tool ships today — `get_recent_deployments` — but it appears nowhere in that designed set, so it does not make this bullet more earned. Do not upgrade this bullet's verb to "Built" or "Implemented" without shipping the rest.
* **Not earned as written.** "Added human-in-the-loop approval controls for state-changing actions to improve agent safety and production readiness." No state-changing action path exists to control — `README.md`'s capability matrix records *Action execution — Not implemented*, and the workflow "**records a decision only** — it never executes, simulates executing, or schedules execution" (`docs/13-approval-workflow.md` §1). An accurate version claims a persisted, auditable human APPROVE/REJECT decision recorded against the agent's typed action proposals, with no execution path behind it — which is what shipped, and is still the substantive safety-boundary point.
* **Partly earned.** "Created eval cases to measure classification accuracy, tool selection quality, citation correctness, hallucination risk, latency, and token cost." The 26-case harness measures citation/evidence correctness, fabricated-evidence rejection, recorded token counts, and deterministic **tool correctness** — a real shipped `toolCorrectness` metric over five checks (`tool-requested`, `tool-executed`, `tool-completed`, and two forbidden-tool checks), which fails if a case's expected call is missing, names the wrong tool, or executes a forbidden one. It does **not** measure ticket-classification accuracy (no such check exists), latency, or monetary cost (`docs/07-evaluation-plan.md` §9 deliberately excludes wall-clock timing and cost). "Tool *selection* quality" specifically is not measured: the provider is fixture-driven, so a case proves the orchestrator honored a declared call, not that a model chose well. Claim "deterministic tool-correctness checks," not "tool-selection quality."
